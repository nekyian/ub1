...
class Arbore
  ...
  public:
    ...
    //////
    //////  Arbore& operator+=(const Arbore& arbore1)
    //////  {
    //////    //...
    //////  }
    //////
    //////
    //////  inline X operator+(Arbore<T> arbore1, const Arbore<T>& arbore2)
    //////  {
    //////  arbore1 += arbore2;
    //////  return arbore1;
    //////  }
  ...
  if(temp != NULL)
    {
      ...
      if (temp->data==element)
        i++; //i+=temp->contor; todo: returnare contor;
...
template <class T>
void Arbore<T>::delnode(node<T>*& root,T element)
{
  node<T> *tmp;
  if (!root)
    cout<<"\n nodul nu exista \n";
  ...
  else if ((element==root->data)&&(root->contor > 1))
    root->contor--;
  //
  else if (element<root->data)
    delnode(root->left,element);
  else if (element>root->data)
    delnode(root->right,element);
  else
    {
      tmp=root;
      if (!tmp->left)
...
//to do: parametrul
template <class T>
T Arbore<T>::operator- (T element) //(Arbore - element)
...
Arbore<T> Arbore<T>::  operator- (Arbore<T> arbore1)
{
  ...
  for (int i=1; i<=arbore1.size; i++)
    *this-arbore1.inord[i];  //arbore1.size=nr de noduri
  return *this;
